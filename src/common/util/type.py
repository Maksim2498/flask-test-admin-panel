...
class AttrInfo:
  ...
  def __placeholder(value_type: type, plural: bool = False) -> str:
    ...
    return (
      ...
      if origin is None
      else AttrInfo.__generic_placeholder(value_type, plural)
    )
...
def cast_text(text: str, target_type: type[T]) -> T:
  ...
  return _cast_text_to_non_generic(text, target_type) if origin is None else _cast_text_to_generic(text, target_type)
